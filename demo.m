 ...
 f_evec=uu*alpha;
 t=toc;
 fprintf('Time for laplacian Eig Vec %f\n',t);
...
% plot out 
 ...
 f_efunc=uu2*alpha2;
 t=toc;
 fprintf('Time without Optimization %f\n',t);
 %%%%%%%%%%%% This line is optimized by the following procedure %%%%%%%%
 tic;
 lambda_value = 1000;
 indexes = find(y~=0);
 temp = zeros(size(uu2));
 temp(indexes,:)=uu2(indexes,:)*lambda_value;
 ...
 f_efunc2=uu2*alpha2;
 t=toc;
 fprintf('Time with Optimization %f\n',t);
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 check = abs(f_efunc - f_efunc2);
 checksum = sum(check)
